-- DRY RUN. Identical to 2026-09-06-move-picana-to-personal.sql except that it
-- raises at the end, which rolls the whole block back. Every guard runs, every
-- UPDATE runs and its row count is asserted, then nothing persists.
--
-- Expected output: NOTICE lines, then
--   ERROR:  DRY RUN COMPLETE - rolled back, nothing was changed
-- That error is SUCCESS. Any other error is a real problem, found for free.
--
...
BEGIN
  ...
  RAISE NOTICE 'DRY RUN: all guards passed, all row counts matched.';
  RAISE NOTICE 'DRY RUN: would move property + matter + 15 documents + lock 2 to % (team %)', dst_org, dst_team;

  /*
    ABORT ON PURPOSE. A DO block is atomic, so raising here undoes every UPDATE,
    INSERT and DELETE above — but only AFTER they have run against real rows and
    every guard and row-count assertion has been checked. That is the whole
    point: a dry run that skipped the writes would not prove the counts match.

    Seeing "DRY RUN COMPLETE - rolled back" below means the real migration will
    succeed. Any other error is a genuine failure, and it has told you before
    you changed anything.
  */
  RAISE EXCEPTION 'DRY RUN COMPLETE - rolled back, nothing was changed';
END $$;
