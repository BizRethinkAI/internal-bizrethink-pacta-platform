...
export type EmailContextResponse = {
  ...
  // NOTE for BizRethink (overlay 010 v2): upstream now exposes organisationId
  // itself, which is what our per-org SMTP routing needs — callers pass it to
  // getMailer(orgId). Our duplicate `string | undefined` field was dropped in
  // the 2026-08-13 sync; do not re-add it.
  organisationId: string;
