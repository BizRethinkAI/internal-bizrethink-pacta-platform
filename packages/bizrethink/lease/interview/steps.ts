    ...
    */
    ...
    fields: [
      ...
      },
      {
        ...
        help: 'Name the one actually installed — an exit alarm on the doors and windows with pool access, an approved safety barrier or fence, an approved powered safety cover, or self-closing self-latching devices. The lease obliges the tenant not to defeat it, so naming the wrong one is worse than naming none.',
        placeholder: 'exit alarms on all doors and windows with direct pool access',
        showWhen: (a) => a.facts.hasPool,
        required: true,
        statute: {
          cite: 'Ch. 515, Fla. Stat.',
          note: 'A residential pool completed after 1 October 2000 must have at least one approved safety feature.',
        },
      },
      {
      ...
      {
        name: 'assessmentsPaidBy',
        target: 'value',
        kind: 'text',
        label: 'Who pays the association’s regular and special assessments?',
        help: 'Almost always the Landlord — they are an owner’s obligation under the declaration. The clause used to bind the tenant to "all of the obligations of the Owner", which on its face handed them the dues; a tenant read that and asked who pays. Now the lease says. Answer in words that will read in a sentence — “Landlord” or “Tenant”.',
        showWhen: (a) => a.facts.hasHoa,
        required: true,
      },
      {
        ...
        target: 'fact',
        kind: 'boolean',
        label: 'Is the property in a community development district (CDD)?',
        help: 'A CDD is a unit of local government under Ch. 190, not the association — separate assessments, usually collected on the property tax bill. A property can be in one, the other, both or neither. If you are not sure, the annual tax bill lists it as a non-ad valorem assessment.',
      },
      {
        name: 'cddName',
        jurisdictions: ['US-FL'],
        target: 'value',
        kind: 'text',
        label: 'What is the district called?',
        showWhen: (a) => a.facts.hasCdd,
        required: true,
      },
      {
        name: 'cddAssessmentsPaidBy',
        jurisdictions: ['US-FL'],
        target: 'value',
        kind: 'text',
        label: 'Who pays the district assessments?',
        help: 'Ordinarily the Landlord, because they arrive on the property tax bill — but ordinarily is not always, and a lease that stays silent is what produced the question. Answer in words that will read in a sentence — “Landlord” or “Tenant”.',
        showWhen: (a) => a.facts.hasCdd,
        required: true,
      },
      {
