...
export const run = async ({ payload, io }: { payload: TSealDocumentJobDefinition; io: JobRunIO }) => {
  ...
  // Upstream 2026-05-25 merge: moved completed-email send out of this handler
  // into a dedicated job. The trigger fires the same conditions (sendEmail +
  // !isResealing + !isRejected, modulo the resealing edge case).
