...
export default function LeaseReviewPage() {
  ...
  */
  ...
  const attachments = ((review.data as { attachments?: Attachment[] }).attachments ?? []) as Attachment[];

  /*
    Split by kind rather than listed together. "The lease requires compliance
    with them" is true of a declaration and false of a condition report — one
    is a rulebook, the other is a photograph of the hallway — and a single
    heading over both would misdescribe whichever it did not mean.
  */
  const governing = attachments.filter((attachment) => attachment.kind === 'hoa-governing');
  const condition = attachments.filter((attachment) => attachment.kind !== 'hoa-governing');
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-10 pb-28">
      ...
      */}
          ...
          {/*
            The documents the lease binds the tenant to, which are not part of
            the lease and are not generated by anything here. The receipt
            addendum names them; this is where they can actually be read, and
            without it that addendum asks for a signature on a list nobody can
            open.
          */}
          {governing.length > 0 && (
            <div className="mt-8 border-t pt-6">
              <h2 className="font-semibold text-lg">Documents referred to in this lease</h2>
              <p className="mt-1 text-muted-foreground text-sm">
                Not part of the lease, and not signed with it. The lease requires compliance with them, and one addendum
                records that you received them.
              </p>

              <ul className="mt-4 space-y-2">
                {governing.map((attachment) => (
                  <li key={attachment.id}>
                    <a
                      className="text-sm underline underline-offset-4"
                      href={`/lease-review/${token}/attachment/${attachment.id}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      {attachment.label}
          ...
          {condition.length > 0 && (
            <div className="mt-8 border-t pt-6">
              <h2 className="font-semibold text-lg">The condition of the property</h2>
              <p className="mt-1 text-muted-foreground text-sm">
                What the property looked like at the start of the term. One addendum records that you received this and
                that it describes the property — so read it, and say if anything is missing. This is the record both
                sides refer to when the deposit is settled.
              </p>

              <ul className="mt-4 space-y-2">
                {condition.map((attachment) => (
                  <li key={attachment.id}>
                    <a
                      className="text-sm underline underline-offset-4"
                      href={`/lease-review/${token}/attachment/${attachment.id}`}
                    ...
                    <span className="ml-2 text-muted-foreground text-xs">
                      {[attachment.documentDate, attachment.pageCount === null ? '' : `${attachment.pageCount} pages`]
                        .filter((part) => part !== '')
                        .join(' · ')}
                    </span>
                  </li>
