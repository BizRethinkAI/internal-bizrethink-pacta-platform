  ...
  const orgGroup: SettingsNavGroup | null = canManageOrg
    ? {
        ...
        items: [
          ...
          // MODIFIED for BizRethink (overlay 008, re-homed here 2026-08-13 when
          // upstream moved nav definitions out of o.$orgUrl.settings._layout.tsx):
          // gate on the claim flag alone. Self-host runs with billing disabled but
          // the BIZRETHINK/PRO claims grant emailDomains, so requiring
          // requiring billing-enabled would hide a feature the org actually has.
          ...(organisation.organisationClaim.flags.emailDomains || isDocumensoCloud
            ? [
                {
                  key: 'email-domains',
          ...
          // MODIFIED for BizRethink (overlay 008, re-homed 2026-08-13): same
          // claim-flag-only gating for the SSO portal entry.
          ...(organisation.organisationClaim.flags.authenticationPortal || isDocumensoCloud
            ? [
                {
                  key: 'sso',
            ...
            : []),
          // ADDED for BizRethink (overlay 010 Phase B, re-homed 2026-08-13):
          // per-organisation SMTP credentials page.
          {
            key: 'smtp',
          ...
          // MODIFIED for BizRethink (overlay 050, re-homed 2026-08-13): the
          // Billing entry is always shown. Pacta resolves billing from the
          // DB-backed admin Stripe config, not NEXT_PUBLIC_FEATURE_BILLING_ENABLED,
          // so the env-var gate would hide billing from paying customers.
          {
            key: 'billing',
