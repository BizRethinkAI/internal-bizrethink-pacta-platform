...
const handleLiveSubscription = async ({
  ...
  // MODIFIED for BizRethink (overlay 052, migrated here 2026-08-13 when upstream
  // folded on-subscription-created/updated into this convergent sync path):
  // read the item-level current_period_end first and fall back to the root-level
  // field. Newer Stripe API versions ("dahlia"+) moved the period to the
  // subscription item; the SDK is pinned to apiVersion 2022-11-15 whose types
  // lack the item-level field, hence the casts.
