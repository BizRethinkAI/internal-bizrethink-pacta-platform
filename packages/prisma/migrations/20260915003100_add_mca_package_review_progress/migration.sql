...
ALTER TABLE "BizrethinkMcaPackageReview" ADD CONSTRAINT "BizrethinkMcaPackageReview_scope_check" CHECK (
  ("kind" = 'library' AND "teamId" IS NULL AND "organisationId" IS NULL AND "templateId" IS NULL AND "templateVersion" IS NULL AND "snapshot"->>'kind' = 'library')
  OR ("kind" = 'provider' AND "teamId" IS NOT NULL AND "organisationId" IS NOT NULL AND "templateId" IS NOT NULL AND "templateVersion" IS NOT NULL AND "templateVersion" > 0 AND "snapshot"->>'kind' = 'provider')
);
