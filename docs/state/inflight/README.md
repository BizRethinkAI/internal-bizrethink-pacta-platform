...
One coordinator-assigned author folds main's notes into STATE.md at the end of
a shipping batch, through a pure `chore/state-consolidation-<batch>` PR. It only
updates STATE.md and deletes notes, and creates no note itself. A different
session or Shwet reviews and merges it. Synthesize current facts, preserve open
limitations and correct stale claims; do not concatenate duplicate accounts.
